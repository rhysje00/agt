#############################################################################
##
#W  vctgrph.gi          Algebraic Graph Theory package         Janoš Vidali
##
##
#Y  Copyright (C) 2020
##
##  Implementation file for functions that construct vector graphs.
##  
##
...
function()
 return ComplementGraph(LatinSquareGraph(CyclicGroup(4)));
end );
...
d -> HalvedGraph(HammingGraph2(d, 2))
);

# The folded d-cube.
InstallGlobalFunction( FoldedCubeGraph,
d -> AntipodalQuotientGraph(HammingGraph2(d, 2))
);

# The folded halved 2d-cube.
InstallGlobalFunction( FoldedHalvedCubeGraph,
d -> AntipodalQuotientGraph(HalvedGraph(HammingGraph2(2*d, 2)))
);
